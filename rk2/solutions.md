...
### Анализ КС-свойства
Полноценного доказательства, к сожалению, получить не удалось. Приведу здесь свои рассуждения.  
При проверке языка на КС-свойство я пытался построить PDA, но возникало противоречие:  
* После первого применения правила $S \to bSSb$ все сводится к тому, чтобы проверить на принадлежность языку  
  слов, порожденных $S_1$ и $S_2$ (обозначим за $S_1$ левый нетерминал $S$, за $S_2$ - правый $S$), но также необходимо проверить
  что $S_1 \neq S_2^R$.
* С помощью одного стека это сделать не получается: при проверке $S_1$ на принадлежность языку со стека приходится снимать
  символы и к концу этой проверки стек будет пустой, так что свойство $S_1 \neq S_2^R$ проверить не выйдет.
* Если стек не освобождать для того, чтобы проверить непалиндромность, тогда не получиться проверить принадлежность  
  языку слова, порожденного $S_1$.

Грамматика языка дает огромную вариативность слов, поэтому "умного" PDA, комбинирующего оба условия, получить не удалось.  

Рассуждения по поводу накачки следующие:  
* Лучшим словом, комбинирующим свойства грамматики и непалиндромности, по моим наблюдениям, является:  
  $ba^nbba^{n+m}bba^mb$, где четность n и m не совпадает, потому что оно выходит из языка при любой накачке любого из  
  блоков $a^n, a^{n+m}, a^m$ (его можно получить из грамматики единственным способом).  
* Однако гарантировать получение палиндрома не удается из-за разной четности блоков $a^n$ и $a^m$:
  если рассматривать слово, классическое для накачки: $ba^pbba^pa^{p+p!}bba^{p+p!}b$, в котором гарантировано  
  получение $p+p!$ при любой накачке любого из подслов $a^p$, прихожу к противоречию с условием "n и m разной четности",
  потому что $p$ и $p+p!$ всегда одной четности.  
* При рассмотрении более простых по структуре слов зависимость всегда получается вложенная.  
* При рассмотрении более сложных по структуре слов растет вариативность получения слов из грамматики и  
  соответственно накачка работает еще хуже: для слов зачастую достаточна суммарная четность $a$ и $b$,  
  но отследить более конкретные паттерны становится проблематично (хотя явно одной четности мало).
  
Таким образом, по моим наблюдениям язык не КС, но формально доказать это не получается.
### Анализ регурности (на всякий случай)    
